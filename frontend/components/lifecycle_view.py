...
def render_lifecycle_timeline(mule_stage: str) -> tuple[str, str, str]:
    ...
    if mule_stage_clean == "NEWLY_RECRUITED":
        ...
        action_sop = "Mandatory KYC refresh and contact holder to confirm transaction origin."
    elif mule_stage_clean == "ACTIVE_MULE":
        step_states = [2, 2, 1, 0, 0]
        current_step = "Active Mule"
        reason = "Frequent watch-list Warn-Flag hits (F3912), regulatory TMS violations (F670), and UPI structuring transactions matching high-density laundering patterns."
        ...
        reason = "Extreme debit/credit volume frequency ratio (F115) coupled with a complete absence of standard retail banking behavior (F2082), signifying active proceeds dispersal."
    ...
    html_lifecycle = (
        ...
        "    text-align: center;\n"
        "}\n"
        ".mule-stage-label.completed {\n"
        "    color: #10b981;\n"
        "}\n"
        ".mule-stage-label.current {\n"
        "    color: #f97316;\n"
        "}\n"
        ".mule-stage-label.future {\n"
        "    color: #64748b;\n"
        "}\n"
        ...
        "    z-index: 1;\n"
        "}\n"
        ".mule-stage-connector.completed {\n"
        "    background-color: #10b981;\n"
        "}\n"
        "@keyframes stagePulse {\n"
        "    0% { transform: scale(1); box-shadow: 0 0 15px rgba(249, 115, 22, 0.6); }\n"
        "    50% { transform: scale(1.08); box-shadow: 0 0 25px rgba(249, 115, 22, 0.9); }\n"
        "    100% { transform: scale(1); box-shadow: 0 0 15px rgba(249, 115, 22, 0.6); }\n"
        "}\n"
        "</style>\n"
    ...
    for idx, stage_name in enumerate(stages):
        ...
        if idx < len(stages) - 1:
            ...
            connector_class = "completed" if state == 2 and next_state != 1 else ("completed" if state == 2 and next_state == 1 else "future")
...
def render_dispatch_controls(account_id: str) -> None:
    """Renders the context-aware investigator dispatching SOP dashboard."""
    st.markdown("##### 🛡️ Investigator Action Center")
    st.caption("Context-aware compliance dispatching system linked to Core Banking webhooks.")
    
    st.markdown("##### 🕵️ SOP Dispatch Controls")
    col_act1, col_act2, col_act3, col_act4, col_act5 = st.columns(5)
    
    btn_mon = col_act1.button("🕵️ Monitor", key=f"btn_mon_{account_id}")
    btn_rev = col_act2.button("📋 Review", key=f"btn_rev_{account_id}")
    btn_esc = col_act3.button("🚀 Escalate", key=f"btn_esc_{account_id}")
    btn_str = col_act4.button("📂 STR", key=f"btn_str_{account_id}")
    btn_frz = col_act5.button("❄️ Freeze", key=f"btn_frz_{account_id}")
    
    if btn_mon:
        st.toast("🕵️ **Monitor Node**: Account telemetry logging enabled. Watchlist records synced.")
        st.success("🕵️ **Monitor Node**: Continuous watching enabled.")
    if btn_rev:
        st.toast("📋 **Initiate Review**: Investigator review workflow scheduled.")
        st.warning("📋 **Initiate Review**: Scheduled workflow dispatcher.")
    if btn_esc:
        st.toast("🚀 **Escalate Case**: Alert dispatched to senior board.")
        st.error("🚀 **Escalate Case**: Priority alert sent.")
    if btn_str:
        st.toast("📂 **Generate STR**: XML payload queued for FIU-IND.")
        st.success("📂 **Generate STR**: compliance payload queued.")
    if btn_frz:
        st.toast("❄️ **Freeze Request**: CBS freeze webhook executed.")
        st.error("❄️ **Freeze Request**: debit freeze locks applied.")
